function countDown() {
  const diasCasamento = new Date('6 December 2024 19:30:00');
...
document.querySelector('.copy-button').addEventListener('click', function () {
  var pixKey = '11963600553'; // Substitua pelo valor da sua chave Pix
...
function sugerirMusica(event) {
  if (input.value.length > 0 || sugerir.value.length > 0) {
    const link = `https://api.whatsapp.com/send/?phone=5511963600553&text=Olá,+gostaria+de+sugerir+a+música+para+a+festa :+${
      input.value
